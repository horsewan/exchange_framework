...
import com.zwan.bitrade.util.Md5;
...
import static com.zwan.bitrade.util.BigDecimalUtils.compare;
import static com.zwan.bitrade.util.BigDecimalUtils.sub;
import static com.zwan.bitrade.util.MessageResult.error;
...
public class WithdrawController {
    ...
    public MessageResult addAddress(String address, String unit, String remark, String code, String aims, @SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) {
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult deleteAddress(long id, @SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) {
    ...
    public MessageResult addressPage(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user, int pageNo, int pageSize, String unit) {
    ...
    public MessageResult queryWithdrawCoin(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) {
        List<Coin> list = coinService.findAllCanWithDraw();
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult withdraw(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user, String unit, String address,
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult withdrawCode(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user, String unit, String address,
        ...
        isTrue(BigDecimalUtils.compare(fee, new BigDecimal(String.valueOf(coin.getMinTxFee()))), sourceService.getMessage("CHARGE_MIN") + coin.getMinTxFee());
        isTrue(BigDecimalUtils.compare(new BigDecimal(String.valueOf(coin.getMaxTxFee())), fee), sourceService.getMessage("CHARGE_MAX") + coin.getMaxTxFee());
        isTrue(BigDecimalUtils.compare(coin.getMaxWithdrawAmount(), amount), sourceService.getMessage("WITHDRAW_MAX") + coin.getMaxWithdrawAmount());
        isTrue(BigDecimalUtils.compare(amount, coin.getMinWithdrawAmount()), sourceService.getMessage("WITHDRAW_MIN") + coin.getMinWithdrawAmount());
        MemberWallet memberWallet = memberWalletService.findByCoinAndMemberId(coin, user.getId());
        isTrue(BigDecimalUtils.compare(memberWallet.getBalance(), amount), sourceService.getMessage("INSUFFICIENT_BALANCE"));
//        isTrue(memberAddressService.findByMemberIdAndAddress(user.getId(), address).size() > 0, sourceService.getMessage("WRONG_ADDRESS"));
        ...
        withdrawApply.setArrivedAmount(BigDecimalUtils.sub(amount, fee));
        ...
        if (amount.compareTo(coin.getWithdrawThreshold()) <= 0 && coin.getCanAutoWithdraw().equals(BooleanEnum.IS_TRUE)) {
            ...
            json.put("fee", fee);
            //预计到账数量
            json.put("arriveAmount", BigDecimalUtils.sub(amount, fee));
            //币种
            json.put("coin", coin);
            //提币地址
     ...
     */
    @GetMapping("record")
    public MessageResult pageWithdraw(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user, int page, int pageSize) {
