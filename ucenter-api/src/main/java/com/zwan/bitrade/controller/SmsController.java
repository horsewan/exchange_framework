...
import com.zwan.bitrade.entity.Country;
...
import com.zwan.bitrade.vendor.provider.SMSProvider;
...
public class SmsController {
    ...
    public MessageResult sendCheckCode(String phone, String country) throws Exception {
        ...
        if (code != null) {
            ...
            if (!BigDecimalUtils.compare(DateUtil.diffMinute((Date) (valueOperations.get(key + "Time"))), BigDecimal.ONE)) {
                return MessageResult.error(localeMessageSourceService.getMessage("FREQUENTLY_REQUEST"));
            }
        }
        ...
        if (result.getCode() == 0) {
            ...
            return MessageResult.success(localeMessageSourceService.getMessage("SEND_SMS_SUCCESS"));
        } else {
    ...
    public MessageResult sendResetTransactionCode(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) throws Exception {
    ...
    public MessageResult setBindPhoneCode(String country, String phone, @SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) throws Exception {
        Member member = memberService.findOne(user.getId());
        ...
        if ("86".equals(member.getCountry().getAreaCode())) {
            if (!ValidateUtil.isMobilePhone(phone.trim())) {
                return MessageResult.error(localeMessageSourceService.getMessage("PHONE_EMPTY_OR_INCORRECT"));
            }
            result = smsProvider.sendVerifyMessage(phone, randomCode);
        } else {
    ...
    public MessageResult updatePasswordCode(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) throws Exception {
        Member member = memberService.findOne(user.getId());
        Assert.hasText(member.getMobilePhone(), localeMessageSourceService.getMessage("NOT_BIND_PHONE"));
        ValueOperations valueOperations = redisTemplate.opsForValue();
    ...
    public MessageResult addAddressCode(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) throws Exception {
        Member member = memberService.findOne(user.getId());
        Assert.hasText(member.getMobilePhone(), localeMessageSourceService.getMessage("NOT_BIND_PHONE"));
        MessageResult result;
        String randomCode = String.valueOf(GeneratorUtil.getRandomNumber(100000, 999999));
    ...
    public MessageResult withdrawCode(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) throws Exception {
        Member member = memberService.findOne(user.getId());
        Assert.hasText(member.getMobilePhone(), localeMessageSourceService.getMessage("NOT_BIND_PHONE"));
        MessageResult result;
    ...
    public MessageResult resetPhoneCode(@SessionAttribute(SysConstant.SESSION_MEMBER) AuthMember user) throws Exception {
        Member member = memberService.findOne(user.getId());
        Assert.hasText(member.getMobilePhone(), localeMessageSourceService.getMessage("NOT_BIND_PHONE"));
        MessageResult result;
        String randomCode = String.valueOf(GeneratorUtil.getRandomNumber(100000, 999999));
